...
class Client:
    ...
    def __init__(self, cookies={}, **kwargs):
        ...
        self.timeout = kwargs.get("timeout", 10)  # 增加超时时间
        ...
        self.kaptcha_url = urljoin(self.base_url, "jwglxt/kaptcha")
        # 配置请求头，与用户的成功请求保持一致
        self.headers = {
            "Accept": "*/*",
            ...
            "Referer": urljoin(self.base_url, "jwglxt/kbcx/xskbcx_cxXskbcxIndex.html?gnmkdm=N2151&layout=default"),
        ...
        # 将传入的cookies设置到Session对象中
        if cookies:
            self.sess.cookies.update(cookies)
            print(f"已将cookies设置到Session中: {dict(self.sess.cookies)}")
        ...
        try:
            ...
            if req_csrf.status_code != 200:
                return {
                    ...
                    "response_text": req_csrf.text,
                    "response_headers": dict(req_csrf.headers)
                }
            # 获取csrf_token
            ...
            if str(doc("input#yzm")) == "":
                ...
                login_data = {
                    ...
                    "mm": encrypt_password,
                    "language": "zh_CN",
                    "time": timestamp  # 添加时间戳参数
                }
                # 请求登录
                req_login = self.sess.post(
                    f"{self.login_url}?time={timestamp}",
    ...
    def get_info(self):
        """获取个人信息"""
        url = urljoin(self.base_url, "jwglxt/xsxxxggl/xsxxwh_cxCkDgxsxx.html?gnmkdm=N100801")
        try:
    ...
    def get_grade(self, year: int, term: int = 0, use_personal_info: bool = False):
        ...
        url = urljoin(
            ...
            else "jwglxt/cjcx/cjcx_cxXsgrcj.html?doType=query&gnmkdm=N305005",
        )
        temp_term = term
        term = term**2 * 3
    ...
    def get_schedule_pdf(self, year: int, term: int, name: str = "导出"):
        ...
        origin_term = term
        term = term**2 * 3
        print(f"学期参数转换: 原始term={origin_term}, 转换后term={term}")
        
        # 尝试获取学生信息以填充modelList
        student_id = ""
        class_name = ""
        try:
            student_info = self.get_info()
            print(f"学生信息获取结果: {student_info}")
            if student_info.get("code") == 1000:
                student_data = student_info.get("data", {})
                student_id = student_data.get("sid", "")
                class_name = student_data.get("class_name", "")
                print(f"获取到的学生ID: {student_id}, 班级名称: {class_name}")
        except Exception as e:
            print(f"获取学生信息失败: {e}")
            
        # 如果获取不到学生信息，使用默认值
        if not student_id:
            # 尝试从cookies或其他地方获取学生ID
            print("警告：未获取到学生ID")
        if not class_name:
            print("警告：未获取到班级名称")
        
        # 使用与用户提供的成功代码完全一致的表单数据格式
        data = {
            # 基础学期信息
            "xnm": str(year),
            "xqm": str(term),
            ...
            # 学生信息模型 - 与成功代码保持一致
            "modelList[0].xnm": str(year),
            "modelList[0].xqm": str(term),
            "modelList[0].xnmc": f"{year}-{year + 1}",
            "modelList[0].xqmmc": str(origin_term),
            "modelList[0].xh_id": student_id,
            "modelList[0].xh": student_id,
            "modelList[0].xm": name,
            "modelList[0].bjmc": class_name,  # 使用获取到的班级名称
            "modelList[0].xsdm": "",
            "modelList[0].kclbdm": "",
            "modelList[0].kclxdm": "",
        ...
        print(f"发送的请求参数: {data}")

        try:
            # 检查并确保cookies包含JSESSIONID和route2
            print(f"当前cookies: {self.cookies}")
            if 'route2' not in self.cookies and 'JSESSIONID' not in self.cookies:
                print("警告：cookies不包含必要的JSESSIONID或route2")
            
            # 许可接口
            print("开始请求许可接口...")
            pilicy_params = {"gnmkdm": "N2151"}
            req_policy = self.sess.post(
                urljoin(self.base_url, "jwglxt/kbdy/bjkbdy_cxXnxqsfkz.html"),  # 确保完整URL
                headers=self.headers,
                data=data,
            ...
            print(f"许可接口响应状态: {req_policy.status_code}")
            if req_policy.status_code != 200:
                print(f"许可接口返回错误: {req_policy.text[:500]}...")
                return {"code": 2333, "msg": "教务系统挂了"}
            doc = pq(req_policy.text)
            if doc("h5").text() == "用户登录":
                print("许可接口返回登录页面，会话已过期")
                return {"code": 1006, "msg": "未登录或已过期，请重新登录"}
            
            # 关键修复：添加1秒延迟，模拟真实用户操作
            print("许可请求成功，等待1秒...")
            import time
            time.sleep(1)
            ...
            req_file = self.sess.post(
                urljoin(self.base_url, "jwglxt/kbcx/xskbcx_cxXsShcPdf.html"),  # 确保完整URL
                headers=self.headers,
                data=data,  # 使用与策略请求相同的数据
                params=file_params,
                cookies=self.cookies,
                timeout=self.timeout,
            )
            print(f"PDF文件接口响应状态: {req_file.status_code}")
            print(f"响应内容类型: {req_file.headers.get('Content-Type')}")
            print(f"响应内容长度: {len(req_file.content)}")
            
            # 检查响应是否为HTML错误页面
            if 'text/html' in req_file.headers.get('Content-Type', ''):
                doc = pq(req_file.text)
                if "错误" in doc("title").text():
                    error = doc("p.error_title").text()
                    print(f"PDF接口返回错误: {error}")
                    return {"code": 998, "msg": error}
            
            # 检查响应是否为PDF
            if 'application/pdf' not in req_file.headers.get('Content-Type', ''):
                print(f"警告: 响应内容类型不是PDF，可能是错误页面")
                print(f"响应内容前200字符: {req_file.content[:200]}")
            
            result = req_file.content  # 二进制内容
            print(f"获取PDF成功，文件大小: {len(result)}字节")
